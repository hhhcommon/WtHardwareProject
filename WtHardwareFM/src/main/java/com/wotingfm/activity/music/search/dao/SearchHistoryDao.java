package com.wotingfm.activity.music.search.dao;
...
public class SearchHistoryDao {
    ...
    public List<History> queryHistory(History history) {
        ...
        Cursor cursor = null;
        try {
            // 执行查询语句 返回一个 cursor 对象
            cursor = db.rawQuery("Select * from history where userid like ? ", new String[]{s});
            // 循环遍历cursor中储存的键值对
    ...
    public void deleteHistory(String news) {
        ...
        String userid = CommonUtils.getUserId(context);
        if (userid != null) {
            db.execSQL("Delete from history where userid like ?and playname=?", new String[]{userid, news});
        } else {
            db.execSQL("Delete from history where playname=?", new String[]{news});
        }
        db.close();
    }

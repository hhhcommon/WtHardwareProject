package com.wotingfm.activity.common.interphone.creategroup.main;
...
import com.wotingfm.activity.common.interphone.creategroup.create.CreateGroupItemActivity;
...
public class CreateGroupMainActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void init() {
        ...
        View relativeCreatePublic = findView(R.id.relative_create_public);               // 创建公开群 无需验证可加入
        relativeCreatePublic.setOnClickListener(this);

        View relativeCreatePrivate = findView(R.id.relative_create_private);             // 创建密码群 需要密码才来加入
        relativeCreatePrivate.setOnClickListener(this);

        View relativeCreateVerification = findView(R.id.relative_create_verification);   // 创建验证群 需要通过验证才能加入

 ...
 *         2016年1月20日
 */
public class FindNewsResultActivityApp extends AppBaseActivity {
            ...
            if (type.equals(StringConstant.FIND_TYPE_PERSON)) {
                ...
                if (!searchstr.trim().equals("")) {
                    if (GlobalConfig.CURRENT_NETWORK_STATE_TYPE != -1) {
                        dialog = DialogUtils.Dialogph(FindNewsResultActivityApp.this, "正在获取数据");
                        PageNum = 1;
                        RefreshType = 1;
                        getfriend();
                    } else {
                        ToastUtils.show_allways(FindNewsResultActivityApp.this, "网络连接失败，请稍后重试");
                    }
                } else {
                    ...
                    if (type.equals(StringConstant.FIND_TYPE_PERSON)) {
                        if (position > 0) {
                            if (UserList != null && UserList.size() > 0) {
                                ...
                                ToastUtils.show_allways(FindNewsResultActivityApp.this, "获取数据异常");
                            }
                        }
                    } else if (type.equals(StringConstant.FIND_TYPE_GROUP)) {
                        if (position > 0) {
                            if (GroupList != null && GroupList.size() > 0) {
                                if (GroupList.get(position - 1).getGroupCreator().equals(CommonUtils.getUserId(context))) {
                                    Intent intent = new Intent(FindNewsResultActivityApp.this, GroupDetailAcitivity.class);
                                    Bundle bundle = new Bundle();
                                    bundle.putSerializable("contact", GroupList.get(position - 1));
                                    bundle.putString("type", "FindNewsResultActivityApp");
                                    intent.putExtras(bundle);
                                    startActivity(intent);
                                } else {
...
//					SessionId = result.getString("SessionId");
                ...
                if (ReturnType != null && ReturnType.equals("1001")) {
                    ...
                    if (UserList != null && UserList.size() > 0) {
                        if (RefreshType == 1) {
                            adapter = new FindFriendResultAdapter(FindNewsResultActivityApp.this, UserList);
                        ...
                        ToastUtils.show_allways(FindNewsResultActivityApp.this, "数据获取失败，请稍候再试");    // json解析失败
                    }
                    ...
                    if (Message != null && !Message.trim().equals("")) {
                        ToastUtils.show_allways(FindNewsResultActivityApp.this, Message);
                    } else {
                        ToastUtils.show_allways(FindNewsResultActivityApp.this, "数据获取失败，请稍候再试");
                    }
                } else {
                    if (RefreshType == 1) {
                        mxlistview.stopRefresh();
...
//					SessionId = result.getString("SessionId");
                ...
                if (ReturnType != null && ReturnType.equals("1001")) {
                    ...
                    if (GroupList != null && GroupList.size() > 0) {
                        if (RefreshType == 1) {
                            adapters = new FindGroupResultAdapter(FindNewsResultActivityApp.this, GroupList);

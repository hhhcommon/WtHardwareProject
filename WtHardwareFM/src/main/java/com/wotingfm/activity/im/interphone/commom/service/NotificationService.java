...
public class NotificationService extends Service {
    ...
    public void add(String type, String imageUrl, String content, String title, String dealTime) {
        ...
        DBNotifyHistory history = new DBNotifyHistory(bjUserId, type, imageUrl, content, title, dealTime, addTime);
    ...
    class MessageReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals("pushnotify")) {
                ...
                L.e("Notification接收器中数据", Arrays.toString(bt) + "");
                try {
                    L.e("Notification接收器中数据", JsonEncloseUtils.btToString(bt) + "");
                } catch (Exception e2) {
                    e2.printStackTrace();
                }
                try {
...
//                                    String dealTime = null;
                                    try {
                                        ...
                                        if (!friendid.trim().equals("") && GlobalConfig.list_person != null && GlobalConfig.list_person.size() > 0) {
...
//                                                    friendUrl = GlobalConfig.list_person.get(i).getPortraitMini();
                                        ...
                                        GroupInfo userInfo = new Gson().fromJson(groupInfo, new TypeToken<GroupInfo>() {}.getType());
//                                        dealTime = data.get("InviteTime") + "";
...
//                                    String imageUrl = null;
                                    try {
                                        ...
                                        JSONObject arg1 = (JSONObject) jsonParser.nextValue();
                                        String groupInfo = arg1.getString("GroupInfo");
                                        String applyUserInfoString = arg1.getString("ApplyUserInfo");
                                        ApplyUserInfo applyUserInfo = new Gson().fromJson(applyUserInfoString, new TypeToken<ApplyUserInfo>() {}.getType());
                                        GroupInfo userInfo = new Gson().fromJson(groupInfo, new TypeToken<GroupInfo>() {}.getType());
...
//                                        dealTime = data.get("ApplyTime") + "";
                                    ...
                                    try {
                                        ...
                                        JSONTokener jsonParser = new JSONTokener(msg);
                                        JSONObject arg1 = (JSONObject) jsonParser.nextValue();
                                        String userInfoString = arg1.getString("UserInfo");

                                        UserInfo userInfo = new Gson().fromJson(userInfoString, new TypeToken<UserInfo>() {}.getType());
                                        groupId = data.get("GroupId") + "";
                                        if (!groupId.trim().equals("") && GlobalConfig.listGroup != null && GlobalConfig.listGroup.size() > 0) {
                                            for (int i = 0; i < GlobalConfig.listGroup.size(); i++) {
                                                if (GlobalConfig.listGroup.get(i).getGroupId().equals(groupId)) {
                                                    groupName = GlobalConfig.listGroup.get(i).getGroupName();
                                                    groupUrl = GlobalConfig.listGroup.get(i).getGroupImg();
                                                }
                                            }
                                        }
                                        String name = userInfo.getUserName();
                                    ...
                                    try {
                                        ...
                                        try {
                                            String userListString = arg1.getString("UserList");
                                            userList = new Gson().fromJson(userListString, new TypeToken<List<UserInfo>>() {
                                            }.getType());
                                        } catch (Exception e) {
                                            e.printStackTrace();
                                        }
                                    ...
                                    try {
                                        ...
                                        if (groupid != null && !groupid.trim().equals("") && GlobalConfig.listGroup != null && GlobalConfig.listGroup.size() > 0) {
                                            for (int i = 0; i < GlobalConfig.listGroup.size(); i++) {
                                                if (GlobalConfig.listGroup.get(i).getGroupId().equals(groupid)) {
                                                    groupname = GlobalConfig.listGroup.get(i).getGroupName();
                                                    groupurl = GlobalConfig.listGroup.get(i).getGroupImg();
...
//								    	UserInfo inviteuserinfo = new Gson().fromJson(inviteuserinfos, new TypeToken<UserInfo>() {}.getType());
    ...
    private void setNewMessageNotification(Context ctx, String msg, String title) {
        ...
        PendingIntent in = PendingIntent.getBroadcast(ctx, 2, pushIntent, PendingIntent.FLAG_UPDATE_CURRENT);
        notification.flags = notification.flags | Notification.FLAG_AUTO_CANCEL;

        notification.contentIntent = in;

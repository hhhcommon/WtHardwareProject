...
import com.wotingfm.common.helper.BytesTransHelper;
						...
						if(audioStream!=null&&audioStream.size()>0){
							...
							synchronized (Lock) {
								if(b!=null&&b.length>0){
									InterPhoneControlHelper.sendFile(b,num++, type, talkId, groupId);
								}
							}
						}
					}else{
						InterPhoneControlHelper.sendLastFile(-num, type, talkId, groupId);

...
import com.wotingfm.ui.music.player.more.programme.adapter.ProgrammeAdapter;
import com.wotingfm.ui.music.player.more.programme.model.DProgram;
    ...
    private void send(String bcid, String time) {
        ...
        VolleyRequest.RequestPost(GlobalConfig.getProgrammeUrl, tag, jsonObject, new VolleyCallback() {
            ...
            protected void requestSuccess(JSONObject result) {
                if (ProgrammeFragment.isCancel()) return;
                try {
                    ReturnType = result.getString("ReturnType");
                    try {
                        String rt = result.getString("ResultList");
                        if (ReturnType != null && ReturnType.equals("1001")) {
                            List<DProgram> dpList = new Gson().fromJson(rt, new TypeToken<List<DProgram>>() {
                            }.getType());
